package top.zhuzhiwen.pigeonmui.aop;
...
public class ComputerOrMobileAdapterAspect {
     ...
     */
    @Pointcut(value = "execution(public * top.zhuzhiwen.pigeonmui.*.controller.*.*(..))")
...
//                log.info("PC端、手机端页面适配器：" + adapter);
                ...
                if (viewName.length()>=8){
                    if (viewName.substring(0,8).equals("redirect")){
